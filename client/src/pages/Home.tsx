...
import {
  Activity, ArrowDownRight, ArrowUpRight, BarChart3, BrainCircuit, BriefcaseBusiness, Building2,
  CalendarDays, Check, ChevronRight, ChevronDown, CircleAlert, CreditCard, FileText, Landmark, LayoutDashboard,
...
export default function Home() {
  ...
  useEffect(() => {
    if (!auth.loading && !auth.isAuthenticated && window.location.pathname !== "/login") navigate("/login");
    if (!auth.loading && auth.isAuthenticated && window.location.pathname === "/login") navigate("/dashboard");
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      <header className="topbar"><button className="icon-btn mobile-menu" onClick={() => setMobileOpen(true)}><Menu size={20} /></button><div className="breadcrumbs"><span>Personal workspace</span><span className="slash">/</span><b>{section}</b></div><div className="top-actions"><div className="sync-state"><span className="sync-dot" />All accounts synced</div><div className="profile-menu-wrap"><button className="top-avatar" onClick={() => setProfileOpen(!profileOpen)}>{initials}</button>{profileOpen && <div className="profile-menu"><div className="profile-menu-head"><div className="avatar">{initials}</div><div><b>{displayName}</b><span>{auth.user?.email ?? "Private workspace"}</span></div></div><button onClick={() => { setProfileOpen(false); setShowSettings(true); }}>Account settings <ChevronRight size={14} /></button><button className="logout-item" onClick={() => auth.logout()}>Log out <ArrowUpRight size={14} /></button></div>}</div></div></header>
      <div className="content-wrap">
        ...
        {section === "Overview" && <>
          <section className="actionable-grid"><div className={`track-card ${data.metrics.financialTrack.status}`}><div className="actionable-head"><div><p className="eyebrow">FINANCIAL TRACK</p><h2>{data.metrics.financialTrack.status === "healthy" ? "You're on track ✦" : "Your track needs attention"}</h2></div><span className="status-pill">{data.metrics.financialTrack.status === "healthy" ? "Healthy" : "Watch"}</span></div><div className="track-amount">{data.metrics.financialTrack.status === "healthy" ? "Ahead by " : "Behind by "}<strong>{money(data.metrics.financialTrack.delta)}</strong></div><p>{data.metrics.financialTrack.cause}</p><div className="track-compare"><span>Actual {money(data.metrics.financialTrack.actual)}</span><span>Target {money(data.metrics.financialTrack.target)}</span></div><div className="track-line"><i style={{ width: `${Math.min(100, data.metrics.financialTrack.actual / data.metrics.financialTrack.target * 100)}%` }} /></div><button className="action-link" onClick={() => document.getElementById("advisor")?.scrollIntoView({ behavior: "smooth" })}>See why <ArrowUpRight size={14} /></button></div><div className="safe-spend-card"><div className="actionable-head"><div><p className="eyebrow">RECOMMENDATION</p><h2>Safe to spend</h2></div><WalletCards size={18} /></div><strong className="safe-amount">{money(data.metrics.safeSpend.amount)}</strong><span className="safe-label">estimated for today</span><p>Based on income, obligations, savings target, and a three-month reserve.</p><small>Recommendation, not guaranteed permission to spend.</small></div><div className="outlook-card"><div className="actionable-head"><div><p className="eyebrow">PROJECTED · BASED ON CURRENT DATA</p><h2>Your 10-year outlook</h2></div><TrendingUp size={18} /></div><div className="outlook-value">{money(data.metrics.outlook.tenYear, true)}</div><p>Potential net worth at your current monthly contribution of <b>{money(data.metrics.outlook.monthlyContribution)}</b>.</p><div className="outlook-milestones"><span><b>{money(data.metrics.outlook.current, true)}</b>Now</span><span><b>{money(data.metrics.outlook.fiveYear, true)}</b>5 years</span><span><b>{money(data.metrics.outlook.tenYear, true)}</b>10 years</span></div><small>{data.metrics.outlook.assumption}</small></div></section>
          <section className="momentum-card"><div><p className="eyebrow">THIS MONTH · FINANCIAL MOMENTUM</p><h2>Meaningful progress, not just numbers</h2></div><div className="momentum-stat"><strong>{money(data.metrics.momentum.surplus)}</strong><span>Monthly surplus</span></div><div className="momentum-stat"><strong>{data.metrics.momentum.savingsRate}%</strong><span>Savings rate</span></div><div className="momentum-stat"><strong>+{money(data.metrics.momentum.netWorthChange)}</strong><span>Net worth movement</span></div><div className="momentum-stat"><strong>{data.metrics.momentum.runway}</strong><span>Months runway</span></div></section>
          <section className="next-action-card"><div className="next-action-icon"><Sparkles size={18} /></div><div><p className="eyebrow mint-eyebrow">ONE THING I’D DO NEXT</p><h2>{data.metrics.nextAction.title}</h2><p>{data.metrics.nextAction.detail}</p></div><button className="action-link" onClick={() => setSection("Goals")}>{data.metrics.nextAction.cta} <ArrowUpRight size={14} /></button></section>
          ...
          <section className="health-strip"><div className="health-score"><p className="eyebrow mint-eyebrow">CREDWISE INTELLIGENCE</p><h2>Financial health</h2><div className="health-score-row"><strong>{data.metrics.financialHealth.score}</strong><span>/ 100<br /><small>calculated score</small></span></div><p>Based on liquidity, savings, debt load, investments, and credit health.</p></div><div className="health-bars">{[["Liquidity", data.metrics.financialHealth.liquidity], ["Savings", data.metrics.financialHealth.savings], ["Debt", data.metrics.financialHealth.debt], ["Investments", data.metrics.financialHealth.investments], ["Protection", data.metrics.financialHealth.protection]].map(([label, value]) => <div className="health-bar-row" key={label as string}><span>{label}</span><div><i style={{ width: `${value}%` }} /></div><b>{value}</b></div>)}</div><div className="signal-stack">{data.metrics.signals.slice(0, 3).map(signal => <div className={`signal-card ${signal.tone}`} key={signal.title}><span>{signal.tone === "healthy" ? "✓" : signal.tone === "watch" ? "!" : "↗"}</span><div><b>{signal.label} · {signal.title}</b><p>{signal.detail}</p></div></div>)}</div></section>

          <section className="dashboard-grid">
            <div className="panel cashflow-panel"><div className="panel-heading"><div><p className="eyebrow">MONEY IN / MONEY OUT</p><h2>Cash flow</h2><p className="chart-subtitle">Monthly income, expenses, and savings</p></div><span className="select-label">Last 6 months</span></div><PremiumCashflow data={profile.cashFlow} /></div>
